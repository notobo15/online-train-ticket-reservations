...
public class TicketAdminController {
    ...
    @PostMapping("/edit/{id}")
    public String update(@PathVariable("id") Integer id, @Valid @ModelAttribute("ticket") Ticket ticket, Model model) {
        ticket.setTicketId(id);

        CarriageSeatMapping csm = mCarriageSeatMappingService
                .getById(ticket.getCarriageSeatMapping().getCarriageSeatId())
                .get();
        ticket.setCarriageSeatMapping(csm);
        ...
        try {
            ticket =  mTicketService.update(ticket);
        }
